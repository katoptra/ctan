...
- After each publish, `texlive.tlpdb.sha512`, a sample of the run's keys and one HTML file
  are read back through the domain and compared with what was uploaded.
...
and carries whatever checksums upstream publishes beside it. `tlmgr` repeats the signature
check on the client, so a tampered mirror is rejected there too.
